...
def segment_sum(check_index, node_left, node_right, node):
    propagation(node, node_left, node_right)
    ...
    mid = (node_left + node_right) // 2
    if check_index <= mid:
        return segment_sum(check_index, node_left, mid, node * 2)
    else:
        return segment_sum(check_index, mid + 1, node_right, node * 2 + 1)
...
for _ in range(M):
    ...
    if nums[0] == 2:
        check_index = nums[1]
        print(segment_sum(check_index, 1, base, 1))